...
<div class="adm-table-wrap">
    <?php if (empty($budgets)): ?>
        <div class="adm-empty-state" style="text-align:center;padding:64px 16px;">
            <i class="bi bi-wallet2" style="font-size:40px;display:block;margin-bottom:12px;opacity:.4;"></i>
    ...
    <?php else: ?>
        <div style="overflow-x:auto;">
            <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    ?>
                    <tr class="adm-row-hover" style="border-bottom:<?= $isLast ? 'none' : '1px solid var(--border,#f0f2f2)' ?>;">

                        <td class="adm-td">
                            <a href="/budgets/show?id=<?= $e($budget['id']) ?>" class="adm-budget-link">
                                <?= $e($budget['name']) ?>
                            </a>
                        ...
                        <td class="adm-td">
                            <div style="display:flex;align-items:center;gap:8px;">
                                ...
                                <div>
                                    <div class="adm-user-name" style="font-weight:500;"><?= $e($budget['owner_name']) ?></div>
                                    <div style="font-size:11px;color:#5c6c75;"><?= $e($budget['owner_email']) ?></div>
                                </div>
                            </div>
                        </td>

                        <td class="adm-td" style="text-align:center;font-weight:600;"><?= $e($budget['member_count']) ?></td>
                        <td class="adm-td" style="text-align:center;font-weight:600;"><?= $e($budget['transaction_count']) ?></td>
        ...
        <div class="adm-table-footer">
            <span><?= count($budgets) ?> budget<?= count($budgets) > 1 ? 's' : '' ?> partagé<?= count($budgets) > 1 ? 's' : '' ?></span>
        </div>
    <?php endif; ?>
</div>
